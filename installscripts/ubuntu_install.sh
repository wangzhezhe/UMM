...
cmake -S ${MGARD_SRC_DIR} -B ${MGARD_BUILD_DIR} \
  ...
  -DCMAKE_PREFIX_PATH="${ZSTD_INSTALL_DIR};${PROTO_INSTALL_DIR}" \
  -Dzstd_DIR=${ZSTD_INSTALL_DIR}/lib/cmake/zstd \
  -DCMAKE_CXX_COMPILER=g++ -DCMAKE_C_COMPILER=gcc
...
if [ -d $UMM_INSTALL_DIR ]; then
    ...
    -DVTK_DIR=${VTK_INSTALL_DIR}/lib/cmake/vtk-9.2 \
    -DCMAKE_PREFIX_PATH="${ZSTD_INSTALL_DIR}/lib/cmake/zstd;${PROTO_INSTALL_DIR}" \
    -Dmgard_DIR=${MGARD_INSTALL_DIR}/lib/cmake/mgard \
    -DMGARD_DIR=${MGARD_INSTALL_DIR}/lib/cmake/mgard \
    -Dzstd_DIR=${ZSTD_INSTALL_DIR}/lib/cmake/zstd \
    -DBUILD_TESTING=OFF \
    -DCMAKE_CXX_COMPILER=g++ -DCMAKE_C_COMPILER=gcc
